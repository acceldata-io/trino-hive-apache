...
docker  run --platform=linux/amd64 -u "$(id -u)" -v "${ROOT_DIR}:/rundir" --rm thrift:0.9.3 thrift -o /rundir/${TARGET_DIR} --gen java:beans,hashcode,generated_annotations=undated /rundir/${SOURCE_DIR}/hive_metastore.thrift
